...
export const Auth = async (req, res, next) => {
  try {
    ...
    if (type !== "Bearer" || !token) return res.status(401).send({ message: "Invalid token format." });

    let decoded;
    const secretKeys = [process.env.JWT_KEY_ADMIN, process.env.JWT_KEY_USER];
    ...
    if (err.name === "TokenExpiredError") return res.status(401).send({ message: "Token expired." });
